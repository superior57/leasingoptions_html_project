...
$('.navigation_lvl-item__3Ts7B').click(function(e) {
    if($(e.target).closest('li').hasClass('navigation_lvl-item__3Ts7B')) {
...
$('.navigation_lvl__2RqqG').click(function(e) {
    if($(e.target).closest('li').hasClass('navigation_lvl__2RqqG')) {
        window.dropdown_history.push($(e.target).closest('li'));
        $(e.target).closest('li').addClass('navigation_is-active__1RKE2');
    }
});
...
$(window).on('click', function() {
    // console.log(window.dropdown_history);
    // window.dropdown_history.map((el) => {
    //     console.log(el[0]);
    // })
})